...
theme_axes_math <- theme_void() + theme(
  ...
  axis.title.x = element_text(hjust = 0.95, margin = margin(0.15, 0, 0, 0, unit = "lines")),
  axis.title.y = element_text(vjust = 0.95, margin = margin(0, 0.15, 0, 0, unit = "lines")),
...
format_pv <- function(pvs) {
  lapply(X = pvs, FUN = format_pvi) %>% unlist()
}

tidy_table <- function(
  x, terms, highlight_row = 1, highlight_color = "black",
  highlight_bold = TRUE, digits = c(NA, 3, 3, 2, 5), title = NULL
) {
  x %>%
    tidy() %>%
    select(1:5) %>%
...
::: {.fragment}
[:]{.hi} Build a toolbox of [*empirical* methods, tools, and skills]{.pink} that combine data and statistical insights to test and/or measure theories and policies.
:::
...
::: {.fragment}
<br>[Q]{.hi}[[v2]{.pink}]{.sub} Is there anything special about *econometrics*?
:::
...
::: {.fragment}
<br>[2\. The methods are usually (relatively) straightforward (after training).]{.purple}
:::
...
This question<sup>✋</sup> describes how you plan to recover/observe *as good as random* assignment of your variable of interest (approximating your ideal experiment) [in real life]{.hi-pink}.
...
```{r, statistical_languages, echo = FALSE, fig.height = 6, fig.width = 9, dev = "svg"}
pop_df <- data.frame(
...
pop_df %<>% mutate(lang = factor(lang, ordered = TRUE))
