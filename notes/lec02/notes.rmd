...
--
<br>.hi-purple[Assignment.sub[1]] Make sure [.mono[R]](https://www.r-project.org/), [.mono[RStudio]](https://www.rstudio.com/products/rstudio/download/#download), and [.mono[Quarto]](https://quarto.org/) are updated/running on your computer.
--
...
.hi-slate[Seminar:] Attend seminars/talks to see different topics/perspectives/styles.
<br>.hi-slate[Lab:] Talk with Shaoda; start deepening R knowledge.
...
1\. The .hi[potential-outcomes framework] defines the .b[causal effect] for individual $i$ as
$$\color{#FFA500}{\tau_i} = \color{#e64173}{\text{Y}_{1i}} - \color{#6A5ACD}{\text{Y}_{0i}}$$

2\. The .hi-slate[fundamental problem of causal inference:] we never observe both $\color{#e64173}{\text{Y}_{1i}}$ and $\color{#6A5ACD}{\text{Y}_{0i}}$ for the same individual.

3\. Naively comparing treated and untreated groups confounds the .hi-orange[treatment effect] with .hi-slate[selection bias]—the groups may differ even *absent* treatment.

4\. .hi[Random assignment] of $\text{D}_i$ eliminates selection bias, giving us
$$\mathop{E}\left[ \text{Y}_{i} \mid \color{#e64173}{\text{D}_{i}=1} \right] - \mathop{E}\left[ \text{Y}_{i}\mid \color{#6A5ACD}{\text{D}_{i}=0} \right] = \mathop{E}\left[ \color{#FFA500}{\tau_i} \right]$$

5\. .hi-slate[Covariates] serve two roles: enforce .b[conditional randomization] and .b[increase precision].

6\. Experiments are not perfect—they still require assumptions—but they set the .hi[benchmark] that observational methods try to approximate.
